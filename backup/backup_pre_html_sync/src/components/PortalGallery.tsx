...
export function PortalGallery({
  ...
  const totalPhotos = albums.reduce((acc, alb) => acc + (alb.images?.length || 0), 0);
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <span className="hidden sm:inline-block ml-2 text-[10px] font-semibold text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full uppercase tracking-wider border border-amber-200/60">
                Portal Galeri
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-500 mt-0.5 line-clamp-1">
                Koleksi album foto interaktif Anda • Klik foto untuk membuka Pratinjau
              </p>
              ...
              <div className="flex items-center gap-5 sm:gap-8 mt-3 text-xs">
                ...
                <div>
                  ...
                  <span className="text-slate-400 text-[11px]">Foto</span>
                </div>
                <div>
                  <span className="font-extrabold text-emerald-600 block text-sm sm:text-base">
                    100%
                  </span>
        ...
        {albums.length === 0 ? (
          ...
          >
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-2.5">
              <button
                onClick={onCreateNewAlbum}
                className="w-full sm:w-auto px-5 py-2.5 bg-slate-900 hover:bg-slate-800 active:scale-95 text-white font-bold text-xs rounded-xl flex items-center justify-center gap-2 shadow-sm transition-all cursor-pointer"
                id="portal-empty-create-btn"
              >
              ...
              {onLoadSampleAlbum && (
                <button
                  onClick={onLoadSampleAlbum}
                  className="w-full sm:w-auto px-4 py-2.5 bg-amber-50 hover:bg-amber-100 border border-amber-200/70 text-amber-800 font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                  id="portal-sample-btn"
                >
                  <Sparkles className="w-3.5 h-3.5 text-amber-600" />
                  <span>Coba dengan Foto Sampel</span>
                </button>
              )}
            </div>
          </div>
            ...
            {albums.map((album) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Photo count badge */}
                  <div className="absolute top-1.5 right-1.5 sm:top-2.5 sm:right-2.5 bg-black/60 backdrop-blur-xs text-white text-[9px] sm:text-[11px] font-bold px-1.5 py-0.5 sm:px-2 sm:py-0.5 rounded-md flex items-center gap-1 z-10">
                ...
                >
                  ...
                  <div className="p-3.5 flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <h3 className="text-xs font-bold text-slate-900 leading-tight truncate max-w-[200px]">
                          {album.title}
                        </h3>
                        <span className="text-[10px] text-slate-400 flex items-center gap-1">
                          <Calendar className="w-2.5 h-2.5" />
                          {formatDate(album.createdAt)} • {album.images?.length || 0} Foto
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="p-3.5 space-y-2">
                    <div className="flex items-center justify-between">
                      <button
                        onClick={() => onSelectAlbum(album)}
                        className="px-4 py-2 bg-slate-900 hover:bg-slate-800 active:scale-95 text-white font-bold text-xs rounded-xl flex items-center gap-1.5 transition-all cursor-pointer"
                      >
                        <Eye className="w-3.5 h-3.5" />
                        <span>Buka Pratinjau Album</span>
                      </button>
