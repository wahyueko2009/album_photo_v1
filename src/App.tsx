...
export default function App() {
  ...
  const initialAlbumId = typeof window !== 'undefined' ? (() => {
    ...
    if (path.startsWith('/album/')) {
      const id = path.replace('/album/', '').split('/')[0].split('?')[0];
      if (id) return id;
    }
    const params = new URLSearchParams(window.location.search);
    const queryAlbum = params.get('album');
    if (queryAlbum) return queryAlbum;
    return null;
  })() : null;

  const [viewerAlbumId, setViewerAlbumId] = useState<string | null>(initialAlbumId);
  const [viewerData, setViewerData] = useState<{ images: ImageItem[]; config: AlbumConfig } | null>(null);
  const [viewerLoading, setViewerLoading] = useState<boolean>(Boolean(initialAlbumId));
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      if (path.startsWith('/album/')) {
        const id = path.replace('/album/', '').split('/')[0].split('?')[0];
        setViewerAlbumId(id || null);
      } else {
        const params = new URLSearchParams(window.location.search);
        const q = params.get('album');
        setViewerAlbumId(q || null);
      }
    };
  ...
  // Fetch or load the shared album when viewerAlbumId is active
  useEffect(() => {
    if (!viewerAlbumId) {
      setViewerData(null);
    ...
    // 1. Try local cache first for instant opening
    try {
      const cached = localStorage.getItem(`kamera_album_${viewerAlbumId}`);
      if (cached) {
    ...
    fetch(`/api/album/${viewerAlbumId}`)
      .then(async (res) => {
        if (!res.ok) {
          throw new Error('Album tidak ditemukan atau sudah kedaluwarsa.');
        }
        return res.json();
      })
      .then((data) => {
        if (isMounted) {
          ...
          setViewerLoading(false);
          try {
            localStorage.setItem(`kamera_album_${viewerAlbumId}`, JSON.stringify(data));
          } catch {
            // Ignore quota errors
          }
        }
      ...
      .catch((err) => {
        if (isMounted) {
          ...
          setViewerError(err.message || 'Gagal memuat album foto.');
          setViewerLoading(false);
        }
      });
  ...
  const handleShareAlbum = async () => {
    ...
    try {
      ...
      // Always construct URL from current browser window.location.origin to ensure proper protocol & domain
  ...
  // RENDER SHARED ALBUM VIEWER IF ACTIVE
  if (viewerAlbumId) {
    if (viewerLoading) {
      return (
        <div className="min-h-screen bg-[#fbfaf7] flex flex-col items-center justify-center p-6 text-center" id="viewer-loading">
          <div className="animate-spin rounded-full h-10 w-10 border-3 border-slate-800 border-t-transparent mb-4" />
          <h2 className="text-base font-bold text-slate-800">Membuka Album Foto...</h2>
          <p className="text-xs text-slate-400 mt-1">Sedang menyiapkan foto kenangan untuk Anda</p>
        </div>
      );
    }
    ...
    if (viewerError || !viewerData) {
      return (
        <div className="min-h-screen bg-[#fbfaf7] flex flex-col items-center justify-center p-6 text-center" id="viewer-error">
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 max-w-sm w-full space-y-4">
            <div className="text-4xl">📁</div>
            <h2 className="text-lg font-bold text-slate-800">Album Tidak Ditemukan</h2>
            <p className="text-xs text-slate-500 leading-relaxed">
              {viewerError || 'Tautan album ini tidak valid atau sudah kedaluwarsa.'}
            </p>
            <button
              onClick={() => {
    ...
    const compiledViewerHTML = compileAlbumHTML(viewerData.images, viewerData.config);

    return (
      <div className="fixed inset-0 w-screen h-screen overflow-hidden bg-black flex flex-col z-50" id="shared-album-viewer">
        ...
        <div className="absolute top-3 left-3 right-3 z-40 flex items-center justify-between pointer-events-none">
          <button
            ...
            className="pointer-events-auto bg-slate-900/80 hover:bg-slate-900 text-white backdrop-blur-md px-3.5 py-1.5 rounded-full text-xs font-bold shadow-lg flex items-center gap-1.5 transition active:scale-95 cursor-pointer border border-white/10"
          >
            <ArrowLeft className="w-3.5 h-3.5" /> Buat Album Baru
          </button>

          <div className="pointer-events-auto flex items-center gap-2">
            <a
              href={`https://api.whatsapp.com/send?text=${encodeURIComponent('Halo! Lihat album foto interaktif kami: ' + window.location.href)}`}
              target="_blank"
              rel="noopener noreferrer"
              className="bg-emerald-600/90 hover:bg-emerald-600 text-white backdrop-blur-md px-3.5 py-1.5 rounded-full text-xs font-bold shadow-lg flex items-center gap-1.5 transition active:scale-95 border border-white/10"
            >
              💬 Bagikan ke WA
            </a>
          </div>
        ...
        {/* Fullscreen Interactive Album iframe */}
        <iframe
          srcDoc={compiledViewerHTML}
          title={viewerData.config.title || 'Album Foto'}
